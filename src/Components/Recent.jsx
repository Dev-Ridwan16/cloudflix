...
export default () => {
    ...
    return (
        <div className={`${isMobile
            ? 'top-[20%] relative h-screen'
            : 'top-[58%]'} 
            max-w-[350px] relative mx-auto md:max-w-[769px] md:top-[45%] lg:max-w-[1000px]`}>
            {
                                    ...
                                    <div key={index} className={`keen-slider__slide number-slide${index + 1} md:block rounded`}>
                                        <div className="bg-none" key={context.id} >
                                            <div className={`${isMobile ? 'w-[200px]' : 'w-[250px]'} relative `}>
                                                <div className="">
                                                    <i className={`fa fa-play-circle absolute  text-[#dc2626] opacity-75 bg-slate-100 
                                                    rounded-full md:translate-x-12 
                                                    ${isMobile 
                                                        ? "text-lg translate-x-[90px] translate-y-[128px]"
                                                        : "text-2xl lg:translate-x-16 translate-y-12"} 
                                                        z-20`
                                                    }></i>
                                                    <img
                                                        src={context.image}
                                                ...
                                                <div className="absolute inset-0 h-10 flex 
                                                    flex-col top-auto text-slate-100 bg-[#696969]
                                                    gap-y-2  px-2">
                                                    <h1 className={`${isMobile
                                                        ? "text-[10px]"
                                                        : "text-[12px]"} bg-opacity-70 opacity-100`}>
                                                                {context.title.length >= 20 
                                                                ? (isMobile ? context.title.slice(0, 20) 
                                                                : context.title.slice(0, 20)).concat("...") 
                                                                : context.title}
                                                            </h1>
                                                    <div className="flex">
